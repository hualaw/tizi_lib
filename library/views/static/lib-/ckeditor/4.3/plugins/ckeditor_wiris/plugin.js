 ...
 * @param object target Target yes
 * @param object element Element double clicked
 */
	...
	if (element.nodeName.toLowerCase() == 'img') {
		if (wrs_containsClass(element, _wrs_conf_imageClassName)) {
			if (!_wrs_int_window_opened) {
				...
				wrs_int_openExistingFormulaEditor(target, isIframe, editor.langCode);//yes
			}
			else {
				_wrs_int_window.focus();
			}
...
function wrs_int_openExistingFormulaEditor(element, isIframe, language) {
	...
	_wrs_int_temporalElement = element;
	_wrs_int_temporalElementIsIframe = isIframe;
	_wrs_int_window = wrs_openEditorWindow(language, element, isIframe);//yes
}
 ...
 * @param string mathml
 */
function wrs_int_updateFormula(mathml, editMode, language) {//alert(1212);
