...
/**
 * Created by 91waijiao on 14-3-3.
 */
var cut_div;  //裁减图片外框div
var cut_img;  //裁减图片
var imgdefw;  //图片默认宽度
var imgdefh;  //图片默认高度
var offsetx = 18; //图片位置位移x
var offsety = -370; //图片位置位移y
var divx = 700; //外框宽度
var divy = 400; //外框高度
var cutx = 640;  //裁减宽度
var cuty = 340;  //裁减高度
...
var zmin = 0.1; //最小比例
var zmax = 1.2; //最大比例
var grip_pos = 0; //拖动块位置0-最左 10 最右
var img_grip; //拖动块
var img_track; //拖动条
var grip_y; //拖动块y值
...
//获得图片裁减位置
function getPosVal(){
    var imgpos = getStylepos(cut_img);//获取对象
    var clipx1 = offsetx - imgpos.x;
    var clipy1 = offsety - imgpos.y;
    var clipwidth = cut_img.width;
    var clipheight = cut_img.height;
    var clipx2 = clipx1 + cutx;
    var clipy2 = clipy1 + cuty;
    var class_id =  $(".clipForm").find(".class_id").val();//回填图片
    var clipid =  $(".clipForm").find(".clipid").val();//回填图片唯一id
    var f_width =  $(".clipForm").find(".f_width").val();//回填图片宽度
    var f_height =  $(".clipForm").find(".f_height").val();//回填图片高度
    var json = {"clipid":clipid,"class_id":class_id,"f_width":f_width,"f_height":f_height,"clipx1":clipx1,"clipy1":clipy1,"clipwidth":clipwidth,"clipheight":clipheight,"clipx2":clipx2,"clipy2":clipy2};
    return json;
}
//获得图片裁减位置
function getcutpos(){
