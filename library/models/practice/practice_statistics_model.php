...
class Practice_Statistics_Model extends Practice_Model{
	...
	public function get_participants_stats($num = 10){
		...
		return $this->db
			->query("select a.`p_c_id`, a.`user_num`, b.`p_c_name` ,c.`grade`, c.`p_c_type` 
                    from `practice_participants_stats` as a 
                    left join `practice_category` as b on a.`p_c_id` = b.`id` 
                    left join `practice_category_info` as c on b.`id` = c.`p_c_id` 
                    where c.`online` = 1 and c.grade in (1,2,3,4,5,6,7) 
                    order by a.`user_num` desc limit 0, {$num}")
			->result_array();
