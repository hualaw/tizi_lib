...
class Parents_Kids extends CI_Model {

    private $_table = 'parents_kids';
    ...
    public function add_kid($user_id, $kid_id, $relation_ship=''){
        ...
        $totalSql = "select count(1) as total from $this->_table where parent_user_id=$user_id and is_del=0";
        ...
        $sql = "select count(1) as count from $this->_table where parent_user_id = $user_id and kid_user_id = $kid_id and is_del = 0 ";
        $count = $this->db->query($sql)->row(0)->count ;
        if($count){ // 重复绑定孩子判断
            return array('status'=>false, 'msg'=>$this->lang->line('dupli_kid'));
        }
        $sql = "select count(1) as count from $this->_table where kid_user_id=$kid_id and is_del=0";
        $count = $this->db->query($sql)->row(0)->count;
        ...
        //replace into 
        $sql = "replace into $this->_table values(0,$user_id,$kid_id,'$relation_ship',0)  ";
    ...
    */
    public function get_kids($user_id){
        $sql="select u1.id, u1.student_id,u1.name from user u left join parents_kids pk on u.id = pk.parent_user_id left join user u1 on u1.id=pk.kid_user_id where u.id = $user_id and pk.is_del = 0";
...
// left join user u1 on u1.id=pk.kid_user_id left join student_data sdata on sdata.uid=pk.kid_user_id left join classes_student cs on cs.user_id=pk.kid_user_id left join classes cls on cls.id = cs.class_id left join classes_schools csch on csch.id = cls.school_id where u.id = '$user_id' and pk.is_del = 0";
    ...
    public function get_parents($kid, $except=false){
        $sql = "select u.name realname, pk.relation_ship relation, pk.parent_user_id pid from user u left join parents_kids pk on pk.parent_user_id=u.id where pk.kid_user_id='$kid' and pk.is_del=0 ";
        if($except){
    ...
    */
    function get_kids_parents($kids){
        if(is_array($kids)){
            ...
            $sql = "select pk.kid_user_id as sid, pk.parent_user_id as pid from parents_kids pk where pk.kid_user_id in ( $kid_str ) and pk.is_del = 0";
    ...
    */
    ...
    function get_relationship($pid, $kid, $is_del=false){
        $sql = "select p.name as p_name , k.name as kid_name , relation_ship as relation from user p , user k ,parents_kids pk where pk.parent_user_id = '$pid' and p.id='$pid' and pk.kid_user_id='$kid' and k.id='$kid' ";
        if($is_del){
            $sql .= " and is_del = 1";
        }else{
            $sql .= " and is_del = 0";
    ...
    public function get_lastinfo($user_id, $fields = "b.*"){
		$result = $this->db->query("select {$fields} from parents_kids as a left join user as b on 
